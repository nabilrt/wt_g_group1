...
function post_n($data)
{
    ...
    try {
        ...
        $stmt1->execute([
            ':ID'=> $data["Renter_ID"],
            ':Notify'=>"There is a new notice from house owner for AD-ID ".$data["AD_ID"]."",
...
function fetchAds($data)
{
    $id = $_SESSION["NID"];
    $conn = db_conn();
    $selectQuery = "SELECT * FROM `unapprovedads` WHERE H_Owner_ID LIKE '%$id%' HAVING decision IS NULL";
    try {
        $stmt = $conn->query($selectQuery);
    } catch (PDOException $e) {
        echo $e->getMessage();
...
function rented_count($id)
{

    $conn = db_conn();
    $sql = "SELECT COUNT(*) FROM basha_vara where Owner_ID like '%$id%'";
    $res = $conn->query($sql);
...
function get_request_c($id)
{

    $conn = db_conn();
    $sql = "SELECT COUNT(*) FROM chats where Owner_ID like '%$id%'";
    $res = $conn->query($sql);
    $count = $res->fetchColumn();
...
function add_renter($data)
{
    ...
    $selectQuery2 = "DELETE FROM `unapprovedads` WHERE `AD_ID` = ?";
    $selectQuery3 = "INSERT into notification (ID, Notify, Time) VALUES (:ID, :Notify, :Time)";
    try {
        $stmt = $conn->prepare($selectQuery);
        $stmt->execute([
        ...
        $stmt2->execute([$data["AD_ID"]]);
        $stmt3=$conn->prepare($selectQuery3);
        $stmt3->execute([

            ':ID'=>$data["Renter_ID"],
            ':Notify'=>"You request for house has been accepted for AD-ID ".$data["AD_ID"].", Welcome.",
            ':Time'=>strval(date("Y-m-d h:i:sa"))
        ]);
    } catch (PDOException $e) {
        echo $e->getMessage();
...
function pay_details($id)
{
    $conn = db_conn();
    $selectQuery = "SELECT * FROM `payments` WHERE Owner_ID LIKE '%$id%'";
    try {
        $stmt = $conn->query($selectQuery);
    } catch (PDOException $e) {
        echo $e->getMessage();
